...
public class PostgreSqlDatabaseFixture : IDatabaseFixture, IAsyncLifetime
{
    private const string LocalDefaultConnectionString =
        "Host=localhost;Port=5432;Database=company_employees_integration_test;Username=company_app;Password=company_dev_password;Timeout=5";

    private const string LocalMasterConnectionString =
        "Host=localhost;Port=5432;Database=company_employees;Username=company_app;Password=company_dev_password;Timeout=3";
    ...
    public IDbProviderPlugin Plugin { get; } = new PostgreSqlProviderPlugin();

    public async Task InitializeAsync()
    {
        var configuredConn = Environment.GetEnvironmentVariable("TEST_POSTGRES_CONNECTION_STRING");
